...
def components_BFS(G):
  ...
  # Nodes_tots guarda els nodes que encara no s'han assignat definitivament
  # a una component. Nodes_revisar fa de cua BFS i Llista_revisats evita repetir nodes.
  Nodes_tots=[]
  Nodes_revisar=[]
  Llista_revisats=[]
  Llista_components=[]
  ...
  while not Buit:
    ...
    while Revisar:
      ...
      # Un cop processat el node actual, l'eliminem del conjunt general i
      # l'afegim a la component que estem construint.
...
def components_DFS(G):
  ...
  # Fem servir les mateixes estructures que a BFS, pero la llista de pendents
  # es tractara com una pila per obtenir un recorregut en profunditat.
  Nodes_tots=[]
  Nodes_revisar=[]
  Llista_revisats=[]
  Llista_components=[]
  ...
  # Igual que a BFS, el bucle extern canvia de component i l'intern explora
  # completament la component en que ens trobem.
  while not Buit:
    while Revisar:
      for node in G.neighbors(Node_actual):
        if node not in Llista_revisats:
...
def experiment_cut_edges(G):
  ...
  if len(Nombre_comp)!=1:
    ...
    # Treballem directament sobre el graf rebut i anem eliminant arestes en un
    # ordre aleatori fins que apareixen dues o mes components.
    Tallar=True
    edges=list(G.edges())
    ...
    while Tallar:
      ...
      # Despres de cada tall, comprovem si encara hi ha una sola component.
      Nombre_comp=components_DFS(G)
      if len(Nombre_comp)!=1:
          Tallar=False
